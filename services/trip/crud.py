...
def assign_driver(db: Session, trip_id: int, driver_id: str, distance: float):
    """Called when ride_match assigns a driver"""
    trip = db.query(Trip).filter(Trip.id == trip_id).first()
    if trip:
        trip.driver_id = driver_id
        trip.status = RideStatus.accepted
        # Example fare calc (₹20 base + ₹10/km)
        trip.fare = 20 + (distance * 10)
        db.commit()
        db.refresh(trip)
    return trip
